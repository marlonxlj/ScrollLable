...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(scroll) name:@"soveic" object:nil];
}
...
//           _timer = [NSTimer scheduledTimerWithTimeInterval:pauseInterval target:self selector:@selector(scroll) userInfo:nil repeats:true];
//            [[NSRunLoop currentRunLoop] run];
//            
//        });
        
//        _timer = [NSTimer scheduledTimerWithTimeInterval:pauseInterval target:self selector:@selector(scroll) userInfo:nil repeats:YES];
//        [_timer fire];
//        [[NSRunLoop mainRunLoop] addTimer:_timer forMode:NSRunLoopCommonModes];
        
        //需要加入手动RunLoop，需要注意的是在NSTimer工作期间self是被强引用的
//        _timer = self.timers;
        _timer = [NSTimer timerWithTimeInterval:pauseInterval target:self selector:@selector(scroll) userInfo:nil repeats:YES];
        // 使用NSRunLoopCommonModes才能保证RunLoop切换模式时，NSTimer能正常工作。

        [[NSRunLoop currentRunLoop] addTimer:_timer forMode:NSRunLoopCommonModes];
        
//		[NSTimer scheduledTimerWithTimeInterval:pauseInterval target:self selector:@selector(scroll) userInfo:nil repeats:YES];
        
//        NSTimer *timer = [NSTimer timerWithTimeInterval:pauseInterval target:self selector:@selector(scroll) userInfo:nil repeats:NO];
